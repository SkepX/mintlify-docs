...
- Sudden belief updates from breaking news
...
- Forced liquidations from short-lived belief spikes
...
<CardGroup cols={2}>
  <Card title="Unrealized PnL" icon="calculator">
    Your position value is calculated using the Mark Price, not the Index Price. This ensures PnL reflects sustained belief movement rather than transient spikes.
  </Card>
  ...
  <Card title="Liquidation Checks" icon="triangle-exclamation">
    Liquidation thresholds reference the Mark Price. Temporary belief spikes cannot trigger liquidation if the Mark Price remains stable.
  </Card>
...
- Maintain stable, continuous trading even during volatile belief updates
...
<Card
  title="Next: Margin & Liquidation"
  icon="arrow-right"
  href="/risk-margin/margin-liquidation"
>
