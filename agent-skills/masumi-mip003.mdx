...
Ascend exposes a **Masumi Adapter** that maps MIP-003 `startJob` calls to Ascend trading API actions. This means any Masumi-compatible agent can research markets, place orders, and manage positions on Ascend without needing direct API key access.

---

## How It Works

1. **Buyer agent calls startJob** - The buyer agent sends a `POST /start_job` to the Ascend Masumi Adapter with an `identifier_from_purchaser` and `input_data` array describing the desired action.
...
|---|---|
| `GET /availability` | Check if the Masumi Adapter is live and accepting jobs |
